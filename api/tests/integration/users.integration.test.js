...
describe('Users API Integration Tests', () => {
    ...
    describe('PUT /api/users/profile', () => {
        it('should update user profile', async () => {
            ...
            // Vérifier que les données sont bien mises à jour via API
    ...
    describe('PATCH /api/users/:id/role (Admin only)', () => {
        it('should change user role as admin', async () => {
            ...
            // Vérifier le changement via GET
            const getUserResponse = await request(app)
                .get(`/api/users/${userId}`)
                .set('Authorization', `Bearer ${adminToken}`)

            expect(getUserResponse.status).toBe(200)
            expect(getUserResponse.body.role).toBe('admin')
        })
    ...
    describe('PATCH /api/users/:id/active (Admin only)', () => {
        it('should activate/deactivate user as admin', async () => {
            ...
            expect(getUserResponse.status).toBe(200)
            expect(getUserResponse.body.active).toBe(false)
        })
    ...
    describe('DELETE /api/users/:id (Admin only)', () => {
        it('should delete user as admin', async () => {
            ...
            // Vérifier que l'utilisateur n'existe plus via GET
            const getUserResponse = await request(app)
                .get(`/api/users/${userId}`)
                .set('Authorization', `Bearer ${adminToken}`)
